...
def login():
    ...
    try:
        ...
        if row==None:
            ...
            loginsp = Frame(root1, bg="gray")
            loginsp.place(x=0,y=0,width=1300,height=850)
            label = Label(root1,text=usernameEntry.get())
            label.place(x=625,y=50)
            root1[geometry] = "height=1200"
...
#window
root1 = Tk()  
root1.geometry('1300x550')  
root1.title('Stationary Management System')
global usernameEntry
root = Frame(root1)
root.place(x=0,y=0,width=1000,height=550)
